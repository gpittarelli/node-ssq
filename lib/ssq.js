...
/*  if (needs_challenge) {
    return function (server, port, callback) {
      get_challenge(server, port, function (err, challenge) {
        if (err) {
          callback(err);
          return;
        }
        console.log(challenge);
        var request_with_challenge = Buffer.concat([request, challenge],
                                      request.length + challenge.length);
        ssq_request(server, port, request_with_challenge,
                    decoder, callback);
      });
    };
  }
